...
from . import deb_installer, appimage_installer, archive_installer
...
def check_and_update(entry: AppEntry) -> CheckResult:
    ...
    try:
        if entry.category == "deb":
            ...
            did_install, message, new_path, new_exe_path = archive_installer.check_and_install(
                entry, resolved, st.get("installed_version"),
                st.get("installed_path", ""), st.get("executable_path", "")
            )
            if did_install:
                state_mod.update_app_state(
                    entry.id, installed_version=resolved.version,
                    installed_path=new_path, executable_path=new_exe_path
                )
        else:
            st = state_mod.get_app_state(entry.id)
    ...
    except (deb_installer.DebInstallError, appimage_installer.AppImageInstallError,
            archive_installer.ArchiveInstallError) as exc:
