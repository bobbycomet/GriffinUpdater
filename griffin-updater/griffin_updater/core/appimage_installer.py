...
def check_and_install(entry, resolved, last_version: str, last_path: str) -> tuple[bool, str, str]:
    ...
    still_present = bool(last_path) and Path(last_path).expanduser().exists()
    ...
    if last_version and not still_present:
        missing_note = (
            f" NOTE: the previously installed file ('{last_path}') is no longer "
            f"there - reinstalling even though the version tag didn't change."
        )
    ...
    try:
        ...
        raise AppImageInstallError(str(exc)) from exc
    ...
    return True, f"{entry.name} updated to {resolved.version}.{missing_note}", str(final_path)
